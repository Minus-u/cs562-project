...
def query():
    ...
    for row in cur:
        if row['year']==2026 and row['state']=='NJ' or row['quant']>50 and row['month']<6:
            key = tuple(row[attr] for attr in ['cust', 'prod'])
            if key not in mf_struct:
                mf_struct[key] = {'avg_quant__sum': 0, 'avg_quant__count': 0, 'max_quant': None}.copy()
            mf_struct[key]['avg_quant__sum'] += row['quant']
            mf_struct[key]['avg_quant__count'] += 1
            if mf_struct[key]['max_quant'] is None or row['quant'] > mf_struct[key]['max_quant']: mf_struct[key]['max_quant'] = row['quant']

    # SCANS 2 to 1: Processing 
    ...
    for key, aggs in mf_struct.items():
        row = {attr: key[j] for j, attr in enumerate(['cust', 'prod'])}
