...
def query():
    ...
    for row in cur:
        if True:
            ...
            if key not in mf_struct:
                mf_struct[key] = {'sum_ny_quant': 0, 'sum_nj_quant': 0, 'sum_ct_quant': 0}.copy()

    # SCANS 2 to 4: Processing 
    # MF SCAN FOR GROUPING VARIABLE ny
    ...
    # MF SCAN FOR GROUPING VARIABLE nj
    cur.scroll(0, mode='absolute')
    for row in cur:
        key = tuple(row[attr] for attr in ['cust'])
        if key in mf_struct:
            if row['state'] == 'NJ':
                mf_struct[key]['sum_nj_quant'] += row['quant']

    # MF SCAN FOR GROUPING VARIABLE ct
    cur.scroll(0, mode='absolute')
    for row in cur:
        key = tuple(row[attr] for attr in ['cust'])
        if key in mf_struct:
            if row['state'] == 'CT':
                mf_struct[key]['sum_ct_quant'] += row['quant']
