...
def query():
    ...
    for row in cur:
        if True:
            ...
            if key not in mf_struct:
                mf_struct[key] = {'sum_x_quant': 0, 'sum_y_quant': 0, 'sum_z_quant': 0, 'avg_x_quant__sum': 0, 'avg_x_quant__count': 0, 'avg_z_quant__sum': 0, 'avg_z_quant__count': 0}.copy()

    # SCANS 2 to 4: Processing 
    # MF SCAN FOR GROUPING VARIABLE x
    cur.scroll(0, mode='absolute')
    for row in cur:
        key = tuple(row[attr] for attr in ['cust'])
        if key in mf_struct:
            if row['state'] == 'NY':
                mf_struct[key]['sum_x_quant'] += row['quant']
                mf_struct[key]['avg_x_quant__sum'] += row['quant']
                mf_struct[key]['avg_x_quant__count'] += 1

    # MF SCAN FOR GROUPING VARIABLE y
    cur.scroll(0, mode='absolute')
    for row in cur:
        key = tuple(row[attr] for attr in ['cust'])
        if key in mf_struct:
            if row['state'] == 'NJ':
                mf_struct[key]['sum_y_quant'] += row['quant']

    # MF SCAN FOR GROUPING VARIABLE z
    cur.scroll(0, mode='absolute')
    for row in cur:
        key = tuple(row[attr] for attr in ['cust'])
        if key in mf_struct:
            if row['state'] == 'CT':
                mf_struct[key]['sum_z_quant'] += row['quant']
                mf_struct[key]['avg_z_quant__sum'] += row['quant']
                mf_struct[key]['avg_z_quant__count'] += 1
    ...
    for key, aggs in mf_struct.items():
        ...
        # APPLY HAVING CLAUSE
        if row['sum_x_quant'] > 2 * row['sum_y_quant'] or row['avg_x_quant'] > row['avg_z_quant']:
            output.append(row)
